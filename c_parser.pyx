...
cpdef tuple parse_xyz_all(bytes filename_bytes, Py_ssize_t every=1):
    ...
    Parsing completo di un file extended XYZ con blocchi:
      ...
      n_atoms
      header (con Lattice="...")
      n_atoms righe: symbol x y z mass vx vy vz
    ...
    # -----------------------------
    # positions / velocities: (kept_frames, n_atoms, 3)
    dims3[0] = kept_frames
    dims3[1] = n_atoms
    dims3[2] = 3
    ...
    if pos_arr is None or vel_arr is None:
        raise MemoryError("Allocazione array pos/vel fallita")
    ...
    try:
        while True:
            ...
            for i in range(n_atoms):
                ...
                # ora leggi i 7 numeri: x y z mass vx vy vz
                # useremo pos_data e vel_data
    ...
    return pos_arr, vel_arr, cell_arr, types_arr
