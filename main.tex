...
\begin{figure}[t]
	\begin{center}
		\includegraphics[width=0.95\columnwidth]{figures/HB-trestati.png}
	\end{center}
	\caption{
	...
	$|0^{*}\rangle$ on a characteristic timescale $\tau_1$, corresponding to the
	first energy transfer process. Subsequently, the system relaxes toward the final
	equilibrated state $|0'\rangle$ on a slower timescale $\tau_2$, which represent
...
\begin{equation}
	T(t) = T_{\infty} + A_1 e^{-t/\tau_1} + A_2 e^{-t/\tau_2},
\end{equation}
...
The characteristic relaxation times extracted from the bi-exponential fits are
summarized in Fig.~\ref{fig:temps_all}. A clear hierarchy of energy
redistribution timescales emerges across the three phases: for the excited O–H stretching mode, the dominant relaxation occurs on a sub-picosecond timescale and exhibits a marked phase dependence: $\tau_1 \approx 280\,\mathrm{fs}$ in liquid water, $\approx 220\,\mathrm{fs}$ in ice~IX, and $\approx 160\,\mathrm{fs}$ in ice~Ih. This indicates that the efficiency of the initial energy transfer from the excited stretching vibration to the surrounding network is strongly influenced by the hydrogen-bond topology.
